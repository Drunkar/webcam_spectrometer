...
void draw() {
  ...
  float x_interval = (wavelength_nm_max - wavelength_nm_min) / float(x_c);
  ...
  float x_map = float(width) /
    (wavelength_nm_max - wavelength_nm_min);
  float y_map = (height - captured_max - margin_chart * 2.5) /
...
void drawScale() {
  float x_unit = float(x_c) / (wavelength_nm_max - wavelength_nm_min);
...
void manualMask(PGraphics pg, PGraphics mask, int alpha) {
  /*
